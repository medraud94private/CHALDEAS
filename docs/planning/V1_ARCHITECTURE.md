...
│  [저장고 (Storage)] ────────────────────────────────────────────────    │
│  │                                                                      │
│  │  • 원문 그대로 + 기계적 분류 (Ingestion 결과물)                      │
│  │  • 사건, 인물, 장소 엔티티 + 관계                                    │
│  │  • 정리역(Librarian)이 판단: "새 선반 필요?" "기존 선반에?"          │
│  │  • 하지만 판단 과정은 결과물에 포함되지 않음                         │
│  │  • 결과물 = 기계적으로 정해진 구성만                                 │
│  │                                                                      │
│  └──────────────────────────────────────────────────────────────────    │
│                         ↓                                               │
...
│  [도서관 (Library)] ────────────────────────────────────────────────    │
│  │                                                                      │
│  │  • 큐레이터가 정리한 Chain, Article, Narrative                       │
│  │  • 새 텍스트를 만들고, 묶음을 만들고, 정리                           │
│  │  • 유저 질문 → 응답이 캐시되어 도서관에 추가                         │
│  │  • 자주 찾는 글 → 승격 (user → cached → featured → system)          │
│  │                                                                      │
│  └──────────────────────────────────────────────────────────────────    │
│                                                                         │
│  [사용자 요청 시]                                                       │
│  1. 도서관에 있으면 → 도서관 자료 제공 (빠름)                           │
│  2. 없으면 → 저장고에서 찾아 큐레이터가 정리 → 도서관에 추가            │
│                                                                         │
│  [CHALDEAS 시각화 - 구분 표시]                                          │
│  • 저장고 마커: 연한 색 (raw data, 아직 정리 안됨)                      │
│  • 도서관 마커: 진한 색 (정리된 콘텐츠, Chain 있음)                     │
│  • 특이점 마커: 강조 표시 (Premium 아티클)                              │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## 1. INGESTION LAYER (정보 수집) → 저장고 구축
...
## 2. QUERY LAYER (큐레이션) → 도서관 구축

### 2.1 역할
- 저장고의 데이터를 읽고 **새 텍스트 생성**
- 정리, 묶음(Chain), 아티클 작성
- 유저 질문에 맞춤 응답 → 도서관에 캐싱
- **Ingestion과 완전히 독립 (저장고 읽기만)**
...
| 레이어 | 역할 | 결과물 | 주요 컴포넌트 |
|--------|------|--------|--------------|
| **Ingestion** | 저장고 구축 | 기계적 분류 (판단 과정 미포함) | Collector, Extractor, Loader |
| **Query** | 도서관 구축 | 새 텍스트, Chain, Article | SHEBA, Curator, ChainGenerator |
| **Database** | 저장고 + 도서관 | 엔티티 + 캐시된 콘텐츠 | PostgreSQL + pgvector |
| **Frontend** | CHALDEAS 시각화 | 저장고/도서관 구분 표시 | Globe, Chat, Detail |

### 저장고 vs 도서관

| 구분 | 저장고 (Storage) | 도서관 (Library) |
|------|-----------------|-----------------|
| 생성자 | Ingestion (정리역) | Query (큐레이터) |
| 내용 | 원문 + 기계적 분류 | 새 텍스트, Chain, Article |
| 마커 | 연한 색 | 진한 색 |
| 판단 | 포함 안됨 | 포함됨 (내러티브) |
